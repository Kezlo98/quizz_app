$(document).ready(function () {
  ...
  setTimeout(function () {
    add_message('Laukik', 'img/demo/av2.jpg',
        'Hello Every one do u want to freindship with me?')
  }, '1000');
  setTimeout(function () {
  ...
  function add_message(name, img, msg, clear) {
    ...
    inner.append('<p id="' + id + '" class="user-' + idname + '">'
        + '<span class="msg-block"><img src="' + img + '" alt="" /><strong>'
        + name + '</strong> <span class="time">- ' + hours + ':' + minutes
        + '</span>'
  ...
  function remove_user(userid, name) {
    i = i + 1;
    $('.contact-list li#user-' + userid).addClass('offline').delay(
        1000).slideUp(800, function () {
      $(this).remove();
    });
    var inner = $('#chat-messages-inner');
    var id = 'msg-' + i;
    inner.append('<p class="offline" id="' + id + '"><span>User ' + name
        + ' left the chat</span></p>');
    $('#' + id).hide().fadeIn(800);
  }
});
