      ...
      if (($.browser.msie && (/(static|relative)/).test(this.css('position')))
        ...
        scrollParent = this.parents().filter(function () {
          return (/(relative|absolute|fixed)/).test(
              $.curCSS(this, 'position', 1)) && (/(auto|scroll)/).test(
              $.curCSS(this, 'overflow', 1) + $.curCSS(this, 'overflow-y', 1)
    ...
    fit: {
      left: function (position, data) {
        ...
        position.left = over > 0 ? position.left - over : Math.max(
            position.left - data.collisionPosition.left, position.left);
      },
      top: function (position, data) {
        ...
        position.top = over > 0 ? position.top - over : Math.max(
            position.top - data.collisionPosition.top, position.top);
      }
    },
...
(function ($, undefined) {
  ...
  $.widget("ui.draggable", $.ui.mouse, {
    ...
    _mouseCapture: function (event) {
      ...
      if (this.helper || o.disabled || $(event.target).is(
          '.ui-resizable-handle')) {
        return false;
      }
    ...
    _mouseStop: function (event) {
      ...
      if ((this.options.revert == "invalid" && !dropped) || (this.options.revert
              == "valid" && dropped) || this.options.revert === true
          || ($.isFunction(this.options.revert) && this.options.revert.call(
    ...
    _convertPositionTo: function (d, pos) {
      ...
      var o = this.options,
          scroll = this.cssPosition == 'absolute' && !(this.scrollParent[0]
              != document && $.ui.contains(this.scrollParent[0],
    ...
    _generatePosition: function (event) {
      ...
      if (this.originalPosition) { //If we are not dragging yet, we won't check for options
        ...
        if (o.grid) {
          ...
          var top = o.grid[1] ? this.originalPageY + Math.round(
          ...
          var left = o.grid[0] ? this.originalPageX + Math.round(
      ...
      $.each(inst.sortables, function (i) {
        ...
        if (this.instance._intersectsWith(this.instance.containerCache)) {
          ...
          if (!this.instance.isOver) {
            ...
            this.instance.currentItem = $(self).clone().removeAttr(
                'id').appendTo(this.instance.element).data("sortable-item",
                true);
    ...
    drag: function (event, ui) {
      ...
      for (var i = inst.snapElements.length - 1; i >= 0; i--) {
        ...
        if (!((l - d < x1 && x1 < r + d && t - d < y1 && y1 < b + d) || (l - d
            < x1 && x1 < r + d && t - d < y2 && y2 < b + d) || (l - d < x2 && x2
            < r + d && t - d < y1 && y1 < b + d) || (l - d < x2 && x2 < r + d
            && t - d < y2 && y2 < b + d))) {
  ...
  $.widget("ui.resizable", $.ui.mouse, {
    ...
    destroy: function () {
      ...
      var _destroy = function (exp) {
        ...
        .removeData("resizable").unbind(".resizable").find(
    ...
    _mouseDrag: function (event) {
      ...
      var data = trigger.apply(this, [event, dx, dy]),
          ie6 = $.browser.msie && $.browser.version < 7, csdif = this.sizeDiff;
    ...
    _respectSize: function (data, event) {
      ...
      var el = this.helper, o = this._vBoundaries,
          pRatio = this._aspectRatio || event.shiftKey, a = this.axis,
    ...
    _change: {
      ...
      w: function (event, dx, dy) {
        var o = this.options, cs = this.originalSize,
 ...
 */
  ...
  $.ui.plugin.add("resizable", "alsoResize", {
    ...
    resize: function (event, ui) {
      var self = $(this).data("resizable"), o = self.options,
          os = self.originalSize, op = self.originalPosition;
      ...
      var delta = {
          ...
          _alsoResize = function (exp, c) {
            $(exp).each(function () {
              var el = $(this), start = $(this).data("resizable-alsoresize"),
                  style = {},
  ...
  $.ui.plugin.add("resizable", "containment", {
    ...
    start: function (event, ui) {
      ...
      // i'm a node, so compute top, left, right, bottom
        ...
        var co = self.containerOffset, ch = self.containerSize.height,
    ...
    resize: function (event, ui) {
      var self = $(this).data("resizable"), o = self.options,
          ps = self.containerSize, co = self.containerOffset, cs = self.size,
          cp = self.position,
      ...
      if (cp.left < (self._helper ? co.left : 0)) {
        self.size.width = self.size.width + (self._helper ? (self.position.left
      ...
      if (cp.top < (self._helper ? co.top : 0)) {
        self.size.height = self.size.height + (self._helper ? (self.position.top
            - co.top) : self.position.top);
        if (pRatio) {
      ...
      var woset = Math.abs(
              (self._helper ? self.offset.left - cop.left : (self.offset.left
                  - cop.left)) + self.sizeDiff.width),
          hoset = Math.abs(
              (self._helper ? self.offset.top - cop.top : (self.offset.top
                  - co.top)) + self.sizeDiff.height);

      var isParent = self.containerElement.get(0) == self.element.parent().get(
              0),
    ...
    stop: function (event, ui) {
      var self = $(this).data("resizable"), o = self.options,
          cp = self.position,
          co = self.containerOffset, cop = self.containerPosition,
          ce = self.containerElement;

      var helper = $(self.helper), ho = helper.offset(),
  ...
  $.ui.plugin.add("resizable", "grid", {
    ...
    resize: function (event, ui) {
      var self = $(this).data("resizable"), o = self.options, cs = self.size,
          os = self.originalSize, op = self.originalPosition, a = self.axis,
          ratio = o._aspectRatio || event.shiftKey;
      o.grid = typeof o.grid == "number" ? [o.grid, o.grid] : o.grid;
      var ox = Math.round((cs.width - os.width) / (o.grid[0] || 1)) * (o.grid[0]
          || 1), oy = Math.round((cs.height - os.height) / (o.grid[1] || 1))
          * (o.grid[1] || 1);
...
(function ($, undefined) {
  ...
  $.widget("ui.selectable", $.ui.mouse, {
    ...
    _mouseDrag: function (event) {
      ...
      var x1 = this.opos[0], y1 = this.opos[1], x2 = event.pageX,
          y2 = event.pageY;
      if (x1 > x2) {
        var tmp = x2;
        x2 = x1;
  ...
  $.widget("ui.sortable", $.ui.mouse, {
    ...
    _mouseCapture: function (event, overrideHandle) {
      ...
      if (this.options.handle && !overrideHandle) {
        ...
        $(this.options.handle, currentItem).find("*").andSelf().each(
            function () {
              if (this == event.target) {
                validHandle = true;
              }
    ...
    _mouseDrag: function (event) {
      ...
      for (var i = this.items.length - 1; i >= 0; i--) {
        ...
        var item = this.items[i], itemElement = item.item[0],
        ...
        if (itemElement != this.currentItem[0] //cannot intersect with itself
            ...
            && (this.options.type == 'semi-dynamic' ? !$.ui.contains(
    ...
    serialize: function (o) {
      ...
      $(items).each(function () {
        ...
        if (res) {
          str.push(
              (o.key || res[1] + '[]') + '=' + (o.key && o.expression ? res[1]
                  : res[2]));
        }
      });
    ...
    _intersectsWithPointer: function (item) {
      ...
      var isOverElementHeight = (this.options.axis === 'x') || $.ui.isOverAxis(
              this.positionAbs.top + this.offset.click.top, item.top, item.height),
          isOverElementWidth = (this.options.axis === 'y') || $.ui.isOverAxis(
    ...
    _refreshItems: function (event) {
      ...
      var queries = [[$.isFunction(this.options.items)
          ? this.options.items.call(this.element[0], event,
              {item: this.currentItem}) : $(this.options.items, this.element),
        this]];
      var connectWith = this._connectWith();
      ...
      if (!o.placeholder || o.placeholder.constructor == String) {
        ...
        o.placeholder = {
          ...
          update: function (container, p) {
            ...
            if (!p.height()) {
              p.height(self.currentItem.innerHeight() - parseInt(
                  self.currentItem.css('paddingTop') || 0, 10) - parseInt(
                  self.currentItem.css('paddingBottom') || 0, 10));
            }
            ;
            if (!p.width()) {
              p.width(self.currentItem.innerWidth() - parseInt(
                  self.currentItem.css('paddingLeft') || 0, 10) - parseInt(
                  self.currentItem.css('paddingRight') || 0, 10));
            }
            ;
          }
        };
    ...
    _generatePosition: function (event) {
      ...
      if (this.originalPosition) { //If we are not dragging yet, we won't check for options
        ...
        if (o.grid) {
          var top = this.originalPageY + Math.round(
              (pageY - this.originalPageY) / o.grid[1]) * o.grid[1];
          ...
          var left = this.originalPageX + Math.round(
              (pageX - this.originalPageX) / o.grid[0]) * o.grid[0];
...
(function ($) {
  ...
  $.widget("ui.menu", {
    ...
    hasScroll: function () {
      return this.element.height() < this.element[$.fn.prop ? "prop" : "attr"](
          "scrollHeight");
    },
    ...
    _resetButton: function () {
      ...
      if (icons.primary || icons.secondary) {
        if (this.options.text) {
          buttonClasses.push(
              "ui-button-text-icon" + (multipleIcons ? "s" : (icons.primary
                  ? "-primary" : "-secondary")));
        }

        if (icons.primary) {
          buttonElement.prepend(
              "<span class='ui-button-icon-primary ui-icon " + icons.primary
              + "'></span>");
        }

        if (icons.secondary) {
          buttonElement.append(
              "<span class='ui-button-icon-secondary ui-icon " + icons.secondary
              + "'></span>");
        }
